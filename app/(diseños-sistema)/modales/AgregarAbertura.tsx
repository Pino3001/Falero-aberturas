...
import { AberturaPresupuestoOption, ColorOption, ColorOptionDefault, CortinaOption, SerieOption } from '@/utils/constants/interfases';
...
type EditarAberturaProps = Partial<{
    acabadoProps: ColorOption[];
    seriesProps: SerieOption[];
    cortinasProps: CortinaOption[];
}>;

export type AgregarAberturaRef = {
    open: () => void;
};
const AgregarAbertura = forwardRef<AgregarAberturaRef, AgregarAberturaProps & EditarAberturaProps>(
    ({ ventanaAEditar, handleDone, handleClose, visible, hideModal, id_col_anterior, acabadoProps, seriesProps, cortinasProps }, ref) => {
        // Propiedades del tema
        const { colors, fonts } = useTheme();
        // elementos del context evitando hacer llamadas a la base de datos
        const { stateBD } = useBD();
        const { acabado: acabadoContext, series: seriesContext, cortinas: cortinasContext } = stateBD;

        // Usar props si existen, de lo contrario usar el contexto
        const acabado = acabadoProps ?? acabadoContext;
        const series = seriesProps ?? seriesContext;
        const cortinas = cortinasProps ?? cortinasContext;

        // combo de tipos de aberturas
        ...
        // setea la abertura a la funcion de respuesta del modal
        const handleSubmit = () => {
            if (validateFields()) {
                handleDone(ventana);
            }
        ...
        // calcula el precio de la abertura seleccionada en el modal, luego se setea al objeto abertura como su nuevo precio unitario
        useEffect(() => {
        ...
        // setea el ancho al objeto aberura
        ...
        // setea el alto al objeto aberura
        const handleAlto = (largo: string) => {
            const nuevoValor = largo.replace(/[^0-9.]/g, '');
            const number = Number.parseInt(nuevoValor);
            setState((prevState) => ({
                ...prevState,
                ventana: { ...prevState.ventana, alto: !Number.isNaN(number) ? number : -1 },
                alto: nuevoValor
            }));
        };

        // Limita las series segun el tipo de abertura
