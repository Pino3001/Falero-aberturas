...
import { TextInput, Text, Divider, FAB, Dialog, Button, Snackbar } from 'react-native-paper';
...
export default function EditNuevoPresupuesto() {
  const { id } = useLocalSearchParams();
  const { colors } = useTheme();
  const [presupuesto, SetPresupuesto] = useState<PresupuestosOption>(PresupuestosOptionDefault);

  const { presupuestosUltimaAct, stateBD, insertarPresupuestoConItemsBDContext, updatePresupuestoBDContext } = useBD(); // Para recargar si hay cambios
  const { series, cortinas, acabado } = stateBD;

  const router = useRouter();

  useEffect(() => {
    {/* Si id no es undefined estoy editando y no creando */ }
    const loadPresupuesto = async () => {
      if (id) {
        const data = await getPresupuestoByID(Number(id));
  ...
  const confirmGuardar = async () => {
    try {
      if (id) {
        ...
        await updatePresupuestoBDContext({ ...presupuesto, precio_total: precioTotalTemp });
  ...
  return (
    <View style={[styles.getStartedContainer, { paddingBottom: id ? 15 : null }]}>
      ...
      <View style={{ marginBottom: 0, paddingBottom: 0 }}>
        <TextInput // Imput nombre de usuario del presupuesto (OBLIFATORIO)
          ...
          style={[styles.input, { marginTop: id ? 120 : 40 }]} // si se usa como nuevo presupuesto un margintop y si se usa para editar otro
      ...
      <View style={{ paddingTop: 0 }}>
        ...
        {presupuesto.ventanas.length > 0 ? (
          <View style={{ flexDirection: 'row', gap: 10, alignContent: 'center', justifyContent: 'center', marginTop: 10, marginBottom: insets.bottom + 70 }}>
            ...
            >{id ? 'Editar' : 'Guardar'}</Button>{/* Si id no es undefined estoy editando y no creando */}
            <View></View>
            <Button
              style={{
              ...
              disabled={(mostrarAbertura) || aberturaEditar !== undefined}
              onPress={handeLimpiar}
            >{id ? 'Cancelar' : 'Limpiar'}</Button>{/* Si id no es undefined estoy editando y no creando */}
          </View>
        ) : null
        }
      </View>
...

      {
        showDialog && showGuardar ?
          <DialogComponent
            Title={id ? 'Editar Presupuesto' : 'Guardar Presupuesto'}
            Content_text={id ? '¿Desea editar el presupuesto?' : '¿Desea guardar el presupuesto?'}
            onCancel={() => {
              setShowDialog(false);
              setShowGuardar(false);
            }}
